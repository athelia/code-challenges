...
async def make_random(idx: int, threshold: int = 6) -> int:
    ...
    while i <= threshold:
        print(c[idx + 1] + f'makerandom({idx})=={i} too low; retrying after {idx + 1} seconds.')
...
async def main():
    ...
    res = await asyncio.gather((make_random(0, 9)), (make_random(1, 8)), (make_random(2, 7)))
    # res -> variable to hold result
    # await -> required to get the result from coroutines
    # asyncio.gather -> async controller
